...
DOCKER_REPO=$REPOSITORY/$SERVER_NAME/$STANDALONE/$GIT_BRANCH
...
docker build -f debug.Dockerfile $DOCKER_TAGS_ARGS --build-arg STANDALONE=true --build-arg SEM_VER=${SEM_VER} --build-arg BUILD_TIMESTAMP=${BUILD_TIMESTAMP} --build-arg GIT_BRANCH=${GIT_BRANCH} --build-arg COMMIT_HASH=${COMMIT_HASH} .
