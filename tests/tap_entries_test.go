...
func TestSystemTapExportEntriesCount(t *testing.T) {
	if testing.Short() {
		t.Skip("ignored system test")
	}
	...
	for _, entriesCount := range tests {
		t.Run(fmt.Sprintf("%d", entriesCount), func(t *testing.T) {
			...
			beforeSendTimestamp := time.Now().UnixNano() / int64(time.Millisecond)
			time.Sleep(30 * time.Second)
			...
			time.Sleep(5 * time.Second)
			afterSendTimestamp := time.Now().UnixNano() / int64(time.Millisecond)

			entriesUrl := fmt.Sprintf("http://localhost:8899/mizu/api/exportEntries?from=%v&to=%v", beforeSendTimestamp, afterSendTimestamp)
			requestResult, requestErr := ExecuteHttpRequest(entriesUrl)
			if requestErr != nil {
				t.Errorf("failed to get entries, err: %v", requestErr)
				return
