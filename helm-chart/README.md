...
| `tap.auth.rolesClaim`                     | Name of the JWT claim (OIDC) or SAML attribute carrying role memberships. Replaces `tap.auth.saml.roleAttribute`. | `role` |
...
| `tap.auth.saml.roleAttribute`             | **Deprecated** — use `tap.auth.rolesClaim`. Parsed for YAML back-compat but ignored by the chart. | `""` |
| `tap.auth.saml.roles`                     | **Deprecated** — use `tap.auth.roles`. Parsed for YAML back-compat but ignored by the chart. | `{}` |
