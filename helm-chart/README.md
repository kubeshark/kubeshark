...
| `tap.capture.dissection.enabled`                    | Set to `true` to have L7 protocol dissection start automatically. When set to `false`, dissection is disabled by default, resulting in almost no resource consumption (e.g. Kubeshark is dormant). This property can be dynamically controlled via the dashboard.      | `true`                                                                                                                                                                                                                                           |
| `tap.capture.dissection.stopAfter`                  | Set to a duration (e.g. `30s`) to have L7 dissection stop after no websocket activity between worker and hub.     | `5m`                                                                                                                                                                                                                                             |
