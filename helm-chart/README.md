...
| `tap.snapshots.cloud.s3.endpoint`                 | Custom endpoint URL for S3-compatible storage (NetApp StorageGRID, Dell ECS, MinIO). When set, the chart adds `SNAPSHOT_AWS_ENDPOINT` to the ConfigMap. | `""`                                                                                                                                                                                                                                             |
| `tap.snapshots.cloud.s3.forcePathStyle`           | Use path-style bucket addressing (`endpoint/bucket/key`). Required by most S3-compatible stores without wildcard DNS. | `false`                                                                                                                                                                                                                                             |
