...
| `tap.auth.rolesClaim`                     | Name of the JWT claim (OIDC) or SAML attribute carrying role memberships. | `groups` |
...
| `tap.metrics.serviceMonitor.enabled`        | Create a Prometheus Operator `ServiceMonitor` for the worker and hub metrics services | `false`                                                                                                                                                                                        |
| `tap.metrics.serviceMonitor.labels`         | Additional labels for the `ServiceMonitor` (e.g. the release label your Prometheus selects on) | `{}`                                                                                                                                                                                 |
| `tap.metrics.serviceMonitor.annotations`    | Additional annotations for the `ServiceMonitor`     | `{}`                                                                                                                                                                                                                                             |
| `tap.metrics.serviceMonitor.interval`       | Scrape interval; empty uses the Prometheus default  | `""`                                                                                                                                                                                                                                             |
| `tap.metrics.serviceMonitor.scrapeTimeout`  | Scrape timeout; empty uses the Prometheus default   | `""`                                                                                                                                                                                                                                             |
| `tap.metrics.serviceMonitor.path`           | Metrics path; empty uses the endpoint default       | `""`                                                                                                                                                                                                                                             |
| `tap.metrics.serviceMonitor.scheme`         | Metrics scheme (`http`/`https`)                     | `""`                                                                                                                                                                                                                                             |
| `tap.metrics.serviceMonitor.honorLabels`    | Honor labels exposed by the scrape target           | `false`                                                                                                                                                                                                                                          |
| `tap.metrics.serviceMonitor.relabelings`    | `relabelings` applied to the scrape endpoint        | `[]`                                                                                                                                                                                                                                             |
| `tap.metrics.serviceMonitor.metricRelabelings` | `metricRelabelings` applied to the scrape endpoint | `[]`                                                                                                                                                                                                                                          |
...
| `scripting.enabled`                       | Enables scripting. Also gates the scripting API: when `false` the Hub answers `/scripts`, `/scripts/exec`, `/jobs` and the AI assistant with 409, and the dashboard hides the scripting UI. Scripting is deployment-wide rather than a per-role capability, so this is the only switch that closes it. | `false` |
