...
func NewRequest(request *api.GenericMessage) (harRequest *har.Request, err error) {
	...
	cookies := BuildCookies(reqDetails["cookies"].([]interface{}))
	...
	var queryString []har.QueryString
...
func NewResponse(response *api.GenericMessage) (harResponse *har.Response, err error) {
	...
	cookies := BuildCookies(resDetails["cookies"].([]interface{}))
...
func NewEntry(item *api.OutputChannelItem) (*har.Entry, error) {
	harRequest, err := NewRequest(&item.Pair.Request)
	if err != nil {
		tap.SilentError("convert-request-to-har", "Failed converting request to HAR %s (%v,%+v)", err, err, err)
		return nil, errors.New("Failed converting request to HAR")
	}

	harResponse, err := NewResponse(&item.Pair.Response)
	if err != nil {
		fmt.Printf("err: %+v\n", err)
	...
	totalTime := item.Pair.Response.CaptureTime.Sub(item.Pair.Request.CaptureTime).Round(time.Millisecond).Milliseconds()
	if totalTime < 1 {
		totalTime = 1
	}
