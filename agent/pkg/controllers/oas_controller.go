...
func GetOASServers(c *gin.Context) {
	m := make([]string, 0)
	oas.GetDefaultOasGeneratorInstance().ServiceSpecs.Range(func(key, value interface{}) bool {
...
func GetOASSpec(c *gin.Context) {
	res, ok := oas.GetDefaultOasGeneratorInstance().ServiceSpecs.Load(c.Param("id"))
	if !ok {
